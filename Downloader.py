...
def downloadFile(file_id, file_name, file_type, path, service):
    ...
    fh.seek(0)
    if file_name == 'Untitled document' and untitled_count > 0:
        with open(os.path.join(path, file_name + '(' + str(untitled_count) + ')' + extention), 'wb') as f:
            f.write(fh.read())
            f.close()
        untitled_count = untitled_count + 1
    else:
        if file_name == 'Untitled document':
            untitled_count = untitled_count + 1
